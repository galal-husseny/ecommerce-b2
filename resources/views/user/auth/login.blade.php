    ...
    <div class="m-lr-auto size-210 bor10 p-lr-70 p-t-55 p-b-70 p-lr-15-lg w-full-md m-b-100">
        ...
        <form method="POST" action="{{ route('users.login') }}">
            ...
            <h4 class="mtext-105 cl2 txt-center p-b-30">
                {{__('messages.user.auth.Login_title')}}
            </h4>
            <!-- Email -->
            <div class="bor8 m-b-20 how-pos4-parent">
                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" id="email" type="email" name="email" value="{{old('email')}}" required autofocus placeholder="{{__('messages.user.auth.Email')}}">
            ...
            <div class="bor8 m-tb-20 how-pos4-parent">
                <input class="stext-111 cl2 plh3 size-116 p-l-62 p-r-30" id="password"type="password" name="password" required autocomplete="current-password" placeholder="{{__('messages.user.auth.Password')}}">
            ...
            <div class="mb-3 form-check d-flex justify-content-around  mt-4">
                <div>
                    ...
                    <label class=" text-sm text-gray-600 dark:text-gray-400" for="remember_me">{{__('messages.user.auth.Remember_me')}}</label>
                </div>
                @if (Route::has('users.password.request'))
                    <a class="m-r-50 underline text-dark text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800"
                        ...
                        {{__('messages.user.auth.forgot_password')}}
                    </a>
                @endif
            </div>

            <button class="flex-c-m stext-101 cl0 size-121 bg3 bor1 hov-btn3 p-lr-15 trans-04 pointer">
                {{__('messages.user.auth.Log_in')}}
            </button>
        </form>
    </div>
</div>
